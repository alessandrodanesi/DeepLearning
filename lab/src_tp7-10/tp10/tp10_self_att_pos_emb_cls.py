...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(train, collate_fn=collate_pad(word2id["__PAD__"], word2id["__CLS__"]),
                              batch_size=BATCH_SIZE, shuffle=True)
    test_loader = DataLoader(test, collate_fn=collate_pad(word2id["__PAD__"], word2id["__CLS__"]),
                             batch_size=BATCH_SIZE, shuffle=False)

    criterion = nn.CrossEntropyLoss()
    model = TransformerModel(EMB_DIM, OUT_DIM, NUM_LAYERS).to(device)
    optimizer = optim.Adam(model.parameters(), lr=LR)
    pos_emb = PositionalEncoding(EMB_DIM, MAX_LEN).to(device)
    ...
    for epoch in range(N_EPOCHS):
        ...
        if SAVE and (epoch % 5 == 0 or epoch == N_EPOCHS - 1) and (epoch > 0):
            torch.save({'epoch': epoch,
                       ...
                       f"models/self_attention_pos_cls_model_nlayers{NUM_LAYERS}_bsize{BATCH_SIZE}_not_same_proj.pth")
